...
import { mockPharmaceuticalShipments } from '@/mocks/api/pharmaceuticals-shipments.mock';
import { 
  filterShipmentData, 
  formatReportData, 
  getReportColumns,
...
const ShipmentsReportPage: React.FC = () => {
  // Filters
  ...
  const reportData = useMemo(() => {
    ...
    const filteredData = filterShipmentData(mockPharmaceuticalShipments, filters);
  ...
  const columns = useMemo(() => {
    const fieldDefinitions = SHIPMENT_REPORT_FIELDS.reduce((acc, field) => ({
      ...acc,
      [field.id]: {
        label: field.label,
        format: field.id.includes('date') || field.id.includes('timestamp')
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Field Customization */}
      <ReportCustomization
        availableFields={SHIPMENT_REPORT_FIELDS}
...
export default ShipmentsReportPage;
